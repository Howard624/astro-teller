...
/**
 * @ai Wrote Code
 * @aidetails Overhauled CSS classes on all <select> elements to strictly define text (black) and background (white) colors. 
 * This ensures readability against browser-default styling, especially in Dark Mode environments. 
 * Implemented robust defense using Tailwind's `text-gray-900` and `bg-white` classes, overriding browser controls.
 *
 * @param {void} No props required for this root page component.
 * @returns {JSX.Element} The rendered form UI with guaranteed readability.
 */
export default function Home() {
  // =====================================================================
  // 1. Reactive State Layer (The Data)
  // =====================================================================
  ...
  // Generate date range arrays for parents (1950-1985)
  const years = Array.from({ length: 36 }, (_, i) => 1950 + i);
  const days = Array.from({ length: 31 }, (_, i) => i + 1);

  // =====================================================================
  // 2. Event-Driven Logic Layer (The Procedural flow)
  // =====================================================================
  const handleStartFortuneTelling = (e: React.FormEvent) => {
    e.preventDefault();

    // Strict validation: make sure all fields are filled
    if (!gender || !birthYear || !birthMonth || !birthDay || !birthTime || !mbti) {
      alert("爸爸妈妈，为了算得准确，请把上面的六个选项都填好哦！❤️");
      return;
    }

    setIsLoading(true);

    // Prepare the perfect payload with 6 domains
    const payload = {
      gender,
      year: birthYear,
      month: birthMonth,
    ...
    console.log("🚀 打包好的命理数据，准备发给大模型:", payload);

    // Mock network delay before LLM backend is connected
    setTimeout(() => {
      setIsLoading(false);
      alert("恭喜！数据打包成功。(请按 F12 在 Console 控制台查收你的命理箱) 🎉");
    }, 1800);
  };

  // =====================================================================
  // 3. Declarative UI Layer (The Render)
  // =====================================================================
  
  // 🏗️ 架构师细节：我们将极高对比度的 CSS 样式封装成一个复用常量，减少代码冗余
  // ✅ 核心修复：这里强行规定了文本深灰 (text-gray-900) 和背景纯白 (bg-white)，坚如磐石
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-4 sm:p-8 bg-stone-50">
      <div className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl p-6 sm:p-12 text-center border-t-8 border-red-500">
        ...
        <h1 className="text-5xl font-extrabold text-red-600 mb-2 tracking-widest">
          玄学大脑
        </h1>
        ...
        <form onSubmit={handleStartFortuneTelling} className="flex flex-col gap-6">
          ...
          <div className="flex flex-col text-left">
            <label className="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
              <span className="text-red-500">*</span> 您的性别 (用于紫微排盘)
            </label>
          ...
          <div className="flex flex-col sm:flex-row gap-6">
            <div className="flex-1 flex flex-col text-left">
              <label className="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
                <span className="text-red-500">*</span> 出生年份
              </label>
            ...
            <div className="flex-1 flex flex-col text-left">
              <label className="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
                <span className="text-red-500">*</span> 出生月份
              </label>
          ...
          {/* Row 2: Day and Chinese Zodiac Time */}
          <div className="flex flex-col sm:flex-row gap-6">
            <div className="flex-1 flex flex-col text-left">
              <label className="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
            ...
            <div className="flex-1 flex flex-col text-left">
              <label className="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
                <span className="text-red-500">*</span> 出生时辰
              </label>
          ...
          <div className="flex flex-col text-left mt-2">
            <label className="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
              <span className="text-red-500">*</span> 16型人格 (MBTI)
            </label>
            <select className={SELECT_CLASSES} value={mbti} onChange={(e) => setMbti(e.target.value)}>
              <option value="" disabled>点击选择您的性格特征 ▼</option>
              <optgroup label="✨ 分析家 (Architect)">
